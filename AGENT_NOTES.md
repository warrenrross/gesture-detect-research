...
2. **Phase 0 follow-ups** — Phase 0's first run is done ([`docs/experiments/phase0-gesture-recognizer.md`](./docs/experiments/phase0-gesture-recognizer.md)) and the mixed result has two cheap follow-ups that haven't run yet: **(a)** rerun the canned Gesture Recognizer on raw webcam frames (not screenshots) to isolate detection failure from classification failure, and **(b)** try MediaPipe Model Maker on a small custom `point_any_direction` class — ~50–200 photos — to see if it closes the gap without the full synthetic pipeline. Both should run before Phase 1a kicks off; the reproducible bundle at [`tests/phase0-gesture-recognizer/`](./tests/phase0-gesture-recognizer/) is the starting point for both.

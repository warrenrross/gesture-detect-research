...
2. **Phase 0 follow-ups** — Phase 0's first run is done ([`docs/experiments/phase0-gesture-recognizer.md`](./docs/experiments/phase0-gesture-recognizer.md)) and the mixed result has two cheap follow-ups that haven't run yet. The Model Maker follow-up is now spec'd in detail at [`docs/experiments/phase0b-model-maker-paths.md`](./docs/experiments/phase0b-model-maker-paths.md) — **Path A** (Warren captures real photos, Computer trains, Warren tests in-game) is the recommended starting move; **Path B** (synthetic renders + Model Maker) is the miniature Phase 1 dry run worth running if Path A is ambiguous. The third open follow-up: rerun the canned Gesture Recognizer on raw webcam frames (not screenshots) to isolate detection failure from classification failure. The reproducible bundle at [`tests/phase0-gesture-recognizer/`](./tests/phase0-gesture-recognizer/) is the starting point for all three.

...
[Phase 0b's Path A](./phase0b-model-maker-paths.md) initially proposed "drop the trained model into Hand_AI and play." Inspection of the current [Hand_AI codebase](https://github.com/warrenrross/Hand_AI) showed that's not actually possible as-described: [`assets/js/handTracker.js`](https://github.com/warrenrross/Hand_AI/blob/main/assets/js/handTracker.js) loads MediaPipe's `HandLandmarker` (landmarks only), and gesture classification happens entirely in [`assets/js/gestures.js`](https://github.com/warrenrross/Hand_AI/blob/main/assets/js/gestures.js) as v2.2 heuristics on those landmarks. There is no classifier head in the running game to swap.
...
1. Fork `warrenrross/Hand_AI` (default branch: `main`).
2. Push the changes to a feature branch.
3. Settings → Pages → enable on the feature branch (or merge to fork's `main` and enable there).
...
Branch: `phase0c-gesture-recognizer` on a Hand_AI fork. Hand_AI's default branch is **`main`** (not `master` — worth flagging because earlier session notes incorrectly recorded it as `master`).
...
| 2 | Branch `phase0c-gesture-recognizer` off `main` in the fork. | `git branch --show-current` returns expected name. |
...
|---|---|---|
| `warrenrross/Hand_AI` | `main` | Original live game. Untouched until Phase 0c proves itself. |
| `warrenrross/Hand_AI-gesture-recognizer-experiment` (planned fork) | `main` | Experiment surface. Pages-deployed. Fast iteration. |
...
- **In `Hand_AI-gesture-recognizer-experiment` fork:** the changes from Steps 3–4 above, on branch `phase0c-gesture-recognizer`, merged to fork's `main` once verified. Pages live.
...
- **In `Hand_AI` (original):** open a PR from the fork's `phase0c-gesture-recognizer` branch into `warrenrross/Hand_AI:main`. This is the polished version of the A/B harness shipped to the live site. v2.2 stays the default; learned-classifier mode is opt-in via the query param. This PR ships separately from any specific trained model.
...
| Fork drifts from upstream Hand_AI over time. | Phase 0c branch is short-lived; if it lives more than a couple of sessions, rebase against upstream `main` before each new round of work. |
...
- [Hand_AI/assets/js/handTracker.js](https://github.com/warrenrross/Hand_AI/blob/main/assets/js/handTracker.js) — the file Phase 0c modifies.
